...
`;
    ...
    async function getLast10MessagesJson(channelId) {
        if (!channelId) return [];
        ...
        const excludedTexts = [
            ...
            "Call declined",
            "Call accepted",
            "Call declined - User is busy"
        ];

        const messages = channel.state.messages
            .filter(msg => msg.text && !excludedTexts.includes(msg.text))
            .slice(-10)
            .reverse();
    ...
    if (userControlBtn) {
        userControlBtn.addEventListener('click', async () => {
            const members = Object.values(activeChannel.state.members).filter(m => m.user.id !== userId);
            const partner = members[0]?.user;
    ...
    async function fetchOptions() {
        ...
        const res = await fetch('/u/report/fetch-options', {
    ...
    if (reportSubmitBtn) {
        reportSubmitBtn.addEventListener('click', async () => {
            ...
            if (activeChannel?.id) {
                try {
                    lastMessages = await getLast10MessagesJson(activeChannel.id);
                } catch (err) {
                    console.error('Failed to fetch last messages for context:', err);
                }
            }
